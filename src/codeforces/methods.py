...
class ApiMethod:
    ...
    def __init__(self, methodName: str, payloadSchema: Dict[str, Any], returnSchema: Tuple[Type, Any], baseUrl: str = CF_API, timeout: float = DEFAULT_TIMEOUT):
...
CF_API_METHODS = {
    'getBlogEntryComments': ApiMethod('blogEntry.comments', {
        'blogEntryId': (int, ...)
    }, (List[Comment], ...)),
    'getBlogEntryView': ApiMethod('blogEntry.view', {
        'blogEntryId': (int, ...)
    }, (BlogEntry, ...)),
    'getContestHacks': ApiMethod('contest.hacks', {
        'contestId': (int, ...),
        'asManager': (bool, None)
    }, (List[Hack], ...)),
    'getContestList': ApiMethod('contest.list', {
        'gym': (bool, None)
    }, (List[Contest], ...)),
    'getContestRatingChanges': ApiMethod('contest.ratingChanges', {
        'contestId': (int, ...)
    }, (List[RatingChange], ...)),
    'getContestStandings': ApiMethod('contest.standings', {
        'contestId': (int, ...),
        'asManager': (bool, None),
        'from': (int, None),
        'count': (int, None),
